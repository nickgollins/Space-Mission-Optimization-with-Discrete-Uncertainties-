...
class LunarLogMissionParameters():
...
class LunarScheduling:
    def __init__(self, 
        ...
        if self.ISRU: raise NotImplementedError("ISRU not implemented yet")
        else: self.ISRU_rate = 0
    ...
    def init_network(self) -> None:
        ...
        self.number_float_payloads = len(self.network_input[0][2])  # Number of float payload types: 0 -perm infrastrusture; 1-maintenance; 2-Consumption; 3-habitat+payload; 4-oxygen; # 5-kerosene   
        ...
        self.IO = range(2)
        if self.restricted_timeline is not None:
            self.T = self.restricted_timeline
            self.real_time = self.restricted_real_time
        else:
            self.T = range(int((max([row[5] for row in self.campaign_reqs])+1) * 2))
        ...
        # arc_cost_matrix_I = [[[[[[0 for t in T] for a in IO] for b in PI] for c in I] for d in I] for n in N]
        ...
        for t in self.T:  # Go back and remove LET arcs that cross trimmed time steps
            if t < 6:
                ...
                if sum(self.arc_exist[0,3,t - self.arc_disc_time[n,0,3,t]] for n in
                    ...
                    self.arc_exist[5,3,t] = 0
                else:
                    self.arc_exist[5,3,t] = 1
            elif t + 3 < max(self.T):
                self.arc_exist[0,5,t] = 0
                if sum(self.arc_exist[0,3,t - self.arc_disc_time[n,0,3,t]] for n in
                    range(self.number_vehicles)) == 0:  # If no more inbound LETs exist at this time, remove the outbound LET
    ...
    def init_demand_matrices(self) -> None:
        ...
        '''
        self.demand_matrix = np.zeros((self.number_nodes, self.number_payload_types, max(self.T)+1, self.number_vehicles), dtype=np.float32)
        self.hold_matrix = np.zeros((self.number_nodes, self.number_payload_types, max(self.T)+1, self.number_vehicles), dtype=np.float32)
        ...
        if self.metaheuristic_decision is None:
            ...
            for pay, row in enumerate(self.campaign_reqs):
                ...
                if int(row[0]) == 1:  # Crew always return
                    self.mutable_hold_matrix[int(row[3]),int(row[0]),0,pay] += int(row[1])  # Hold at destination
                    # self.mutable_demand_matrix[int(row[2]),int(row[0]),0,pay] = -int(row[1])  # Demand at origin
            ...
            # print(self.demand_matrix[2, 5, 0, 0])
            # Vehicle supply
            sorted_unique_timeline = np.unique(np.sort(self.metaheuristic_decision))
            for n in range(self.spacecraft.number_non_stack_vehicles):
                for t in range(self.spacecraft.available_from[n], max(sorted_unique_timeline)+1, self.spacecraft.launch_frequency[n]):
                     trimmed_index = min(np.where((sorted_unique_timeline >= t))[0]) * 2   # Even timestep
                     self.demand_matrix[0,0,trimmed_index,n] += 1

            for pay, row in enumerate(self.campaign_reqs):
                time_index = self.metaheuristic_decision[pay]
                trimmed_index = np.where(sorted_unique_timeline == time_index)[0][0] * 2  # Even timestep
                if int(row[2]) < int(row[3]):  # Outbound flow      
                    self.demand_matrix[int(row[2]),int(row[0]),trimmed_index,0] += np.float32(row[1])  # Supplies
                    if int(row[0]) == 1:  # Crew always return
                        self.hold_matrix[int(row[3]),int(row[0]),trimmed_index,0] += int(row[1])  # Hold at destination
                        self.demand_matrix[int(row[2]),int(row[0]),trimmed_index+1,0] -= int(row[1])  # Demand at origin
                    elif int(row[0]) == 2:  # Permanent infrastructure remains until end of mission
                        for t in range(trimmed_index, max(self.T)+1):
                            self.hold_matrix[int(row[3]),int(row[0]),t,0] += int(row[1]) # Hold at destination at required time
                        self.demand_matrix[int(row[3]),int(row[0]),self.T[-1],0] -= int(row[1]) # Demand at destination at end of mission
                    else:
                        self.demand_matrix[int(row[3]),int(row[0]),trimmed_index,0] -= np.float32(row[1]) # Demand
                elif int(row[2]) > int(row[3]):  # Inbound flow never uses LETs
                    trimmed_index = np.where(sorted_unique_timeline == time_index)[0][0] * 2 + 1  # Odd timestep
                    self.hold_matrix[int(row[2]),int(row[0]),trimmed_index,0] += np.float32(row[1])  # Supplies    
                    self.demand_matrix[int(row[3]),int(row[0]),trimmed_index,0] += np.float32(row[1])  # Demand
        return 

    def init_vars(self, names=False) -> None|list:
        self.xI_index = tuplelist([(n, i, j, c, io, t) for n in self.N for i in self.I for j in self.I for c in self.CI for io in self.IO for t in self.T 
                                   if self.valid_arc[n,i,j,t]])
        self.xI_cost = {}
        self.xF_index = tuplelist([(n, i, j, c, io, t) for n in self.N for i in self.I for j in self.I for c in self.CF for io in self.IO for t in self.T 
                                   if self.valid_arc[n,i,j,t]])
        self.xF_cost = {}
        self.B_index = tuplelist([(pay, t) for pay in range(self.number_payloads) for t in self.T])
        for ind in self.xI_index:
            self.xI_cost[ind] = self.arc_cost_matrix_I[ind]
        for ind in self.xF_index:
            self.xF_cost[ind] = self.arc_cost_matrix_F[ind]
    ...
    def init_schedule(self) -> None:
        ...
        '''
        self.soft_precursors = [[] for _ in self.P]
        self.strict_precursors = [[] for _ in self.P]
        self.co_payloads = [[] for _ in self.P]
        for pay in self.P:
    ...
    def build_primal_model(self, method:str,
        ...
        assert method == 'binary' or method == 'metaheuristic', "Method must be either 'binary' or 'metaheuristic'"
        assert method == 'binary' or (subproblem is None and relaxed is False and subsolution is False and prev_MILP_solution is False and gen_var==None), "Subproblem, subsolution shouldn't be defined if method is not 'binary'"
        ...
        if method == 'binary':
            ...
            model = self.build_metaheuristic_scheduling()
    ...
    def build_metaheuristic_scheduling(self) -> Model:
        """Builds the scheduling model based on a structure chosen by the metaheuristic algorithm.
        
        """
        model = Model('Model')

        xI, xF = self.add_vars(model)
        self.add_constrs(model, 'metaheuristic', xI, xF)
    ...
    def build_discrete_scheduling(self,
        ...
        xI, xF, B = self.add_vars(model, subproblem, relaxed, subsolution, B_index)
        self.add_constrs(model, 'binary', xI, xF, B=B, B_index=B_index)
    ...
    def add_vars(self, 
                 ...
                 B_index:Optional[tuplelist]=None
                ) -> Var:
    ...
    def add_constrs(self, 
        ...
        assert method == 'binary' or method == 'metaheuristic', "Method must be either 'binary' or 'metaheuristic'"
        if method == 'binary': 
            assert B_index is not None, "B_index must be defined if method is 'binary'"
            assert B is not None, "B variables must be defined if method is 'binary'"

            model = self.add_binary_scheduling_constrs(model, B, self.B_index)
            model = self.add_demands_constrs(model, xI, xF, B, B_index)
        elif method == 'metaheuristic':
            model = self.add_demands_constrs(model, xI, xF)
    ...
    def add_binary_scheduling_constrs(self, 
        ...
        # Soft precursors
        model.addConstrs((sum(binary_vars[pay, time] for time in range(t+1) if (pay,time) in B_index) <= sum(binary_vars[pre, time] for time in range(t+1) if (pre,time) in B_index)
                        for pay in self.P for t in self.T for pre in self.soft_precursors[pay]
                        ),
                        name='2A')
        # Strict precursors
        model.addConstrs((sum(binary_vars[pay, time] for time in range(t+1) if (pay,time) in B_index) <= sum(binary_vars[pre, time] for time in range(t-1) if (pre,time) in B_index)
                        for pay in self.P for t in self.T for pre in self.strict_precursors[pay]
                        ),
                        name='2B')
        #Co-payloads
        model.addConstrs((binary_vars[pay, t] == binary_vars[co,t] 
                        for pay in self.P for t in self.T for co in self.co_payloads[pay]
                        if (pay,t) in B_index and self.campaign_reqs[pay][3] > self.campaign_reqs[pay][2] 
                        and co != '' and (co,t) in B_index),
                        name='2C_outbound')
        model.addConstrs((binary_vars[pay, t] == binary_vars[co,t-1] 
                        for pay in self.P for t in self.T for co in self.co_payloads[pay]
                        if (pay,t) in B_index and self.campaign_reqs[pay][3] < self.campaign_reqs[pay][2] 
                        and co != '' and (co,t) in B_index and self.campaign_reqs[co][3] > self.campaign_reqs[co][2]),
        ...
        # Launch windows
        model.addConstrs((binary_vars[pay, t] == 0 
                        for pay in self.P for t in self.T 
                        if (pay, t) in B_index and (t < self.campaign_reqs[pay][4]*2 or t > self.campaign_reqs[pay][5]*2 
    ...
    def add_demands_constrs(self,
        ...
        # Vehicles
        if self.stacks == []:
            model.addConstrs((sum(xI[n, i, j, 0, 0, t] for j in self.I if (n, i, j, 0, 0, t) in self.xI_index) - sum(xI[n, j, i, 0, 1, t - self.arc_disc_time[n,j,i,t]] for j in self.I 
                            if (n, j, i, 0, 1, t - self.arc_disc_time[n,j,i,t]) in self.xI_index) <= int(self.demand_matrix[i,0,t,n]) 
                            for i in self.I for t in self.T for n in self.N),
                            name='1A')
        else:
            model.addConstrs((sum(xI[n, i, j, 0, 0, t] for j in self.I if (n, i, j, 0, 0, t) in self.xI_index) 
                                + sum(sum(xI[int(self.non_stack_vehicles+stack_num), i, j, 0, 0, t] for j in self.I if (int(self.non_stack_vehicles+stack_num), i, j, 0, 0, t) in self.xI_index) 
                                    for stack_num in range(len(self.stacks)) if n in self.stacks[stack_num])
                                - sum(xI[n, j, i, 0, 1, t-self.arc_disc_time[n,j,i,t]] for j in self.I if (n, j, i, 0, 1, t-self.arc_disc_time[n,j,i,t]) in self.xI_index)
                                - sum(sum(xI[int(self.non_stack_vehicles+stack_num), j, i, 0, 1, t-self.arc_disc_time[n,j,i,t]] for j in self.I if (int(self.non_stack_vehicles+stack_num), j, i, 0, 1, t-self.arc_disc_time[n,j,i,t]) in self.xI_index)
                                    for stack_num in range(len(self.stacks)) if n in self.stacks[stack_num])
                            ...
                            name='1A')
        if B is not None:
            # Crew demand and return                
            for i, t,in zip(*[list(range(self.number_nodes)), list(range(max(self.T)+1))]):
                model.addConstr(sum(sum(xI[n, i, j, 1, 0, t] for j in self.I if (n, i, j, 1, 0, t) in self.xI_index) for n in self.N) 
                                        - sum(sum(xI[n, j, i,1, 1, t - self.arc_disc_time[n,j,i,t]] for j in self.I if (n, j, i, 1, 1, t - self.arc_disc_time[n,j,i,t]) in self.xI_index) for n in self.N)
                                    <= sum(self.demand_matrix[i,1,t,n] + sum((B[p, t]-B[p,t-1])*float(self.mutable_demand_matrix[i,1,n,p]) for p in self.P if (p,t) in B_index and (p,t-1) in B_index) for n in self.N) 
                                )
            
            # Continuous payloads
            model.addConstrs((sum(sum(xF[n, i, j, c, 0, t] for j in self.I if (n, i, j, c, 0, t) in self.xF_index) for n in self.N) 
                                    - sum(sum(xF[n, j, i, c, 1, t - self.arc_disc_time[n,j,i,t]] for j in self.I if (n, j, i, c, 1, t - self.arc_disc_time[n,j,i,t]) in self.xF_index) for n in self.N)
                                <= sum(self.demand_matrix[i,c+self.number_int_payloads,t,n] for n in self.N) 
                                    + sum(sum(B[p, t]*self.mutable_demand_matrix[i,c+self.number_int_payloads,n,p] for p in self.P if (p,t) in B_index) for n in self.N) 
                                for i in self.I for t in self.T for c in self.CF),
                                name='1B_cont')
            
            # Hold constraints
            model.addConstrs((sum(xI[n, i, i, c, t % 2, t - t % 2]  for n in self.N if (n, i, i, c, t % 2, t - t % 2) in self.xI_index)     
                                >= sum(sum(B[p, t]*self.mutable_hold_matrix[i,c,n,p] for p in self.P if (p,t) in B_index) for n in self.N) 
                                for i in self.I for t in self.T for c in self.CI if c > 0),
                                name='1C_ints')
            
            model.addConstrs((sum(xF[n, i, i, 0, t % 2, t - t % 2] for n in self.N if (n, i, i, 0, t % 2, t - t % 2) in self.xF_index) 
                                >= sum(sum(sum(B[p, t_prime] for t_prime in range(t+1) if (p,t_prime) in B_index)*self.mutable_hold_matrix[i,2,n,p] for p in self.P) for n in self.N) 
                                for i in self.I for t in self.T),
                                name='1C_infra')
            
            model.addConstrs((sum(xF[n, i, i, c, t % 2, t - t % 2] for n in self.N if (n, i, i, c, t % 2, t - t % 2) in self.xF_index) 
                                >= sum(sum(sum(B[p, t_prime] for t_prime in range(t+1) if (p,t_prime) in B_index)*self.mutable_hold_matrix[i,c+self.number_int_payloads,n,p] for p in self.P) for n in self.N) 
                                for i in self.I for t in self.T for c in self.CF[1:]),
                                name='1C_other_cont')
        else:
            # Other integer payloads
            model.addConstrs((sum(sum(xI[n, i, j, c, 0, t] for j in self.I if (n, i, j, c, 0, t) in self.xI_index) for n in self.N) 
                            - sum(sum(xI[n, j, i, c, 1, t - self.arc_disc_time[n,j,i,t]] for j in self.I if (n, j, i, c, 1, t - self.arc_disc_time[n,j,i,t]) in self.xI_index) for n in self.N)
                                <= float(sum(self.demand_matrix[i,c,t,n] for n in self.N))
                            for i in self.I for t in self.T for c in self.CI
                            if c > 0),
                            name='1B_ints')
            # Continuous payloads
            model.addConstrs((sum(sum(xF[n, i, j, c, 0, t] for j in self.I if (n, i, j, c, 0, t) in self.xF_index) for n in self.N) 
                            - sum(sum(xF[n, j, i, c, 1, t - self.arc_disc_time[n,j,i,t]] for j in self.I if (n, j, i, c, 1, t - self.arc_disc_time[n,j,i,t]) in self.xF_index) for n in self.N)
                                <= float(sum(self.demand_matrix[i,c+self.number_int_payloads,t,n] for n in self.N)) 
                            for i in self.I for t in self.T for c in self.CF),
                            name='1B_cont')
            
            # Hold constraints
            model.addConstrs((sum(xI[n, i, i, c, t % 2, t - t % 2]  for n in self.N if (n, i, i, c, t % 2, t - t % 2) in self.xI_index)     
                                >= int(sum(self.hold_matrix[i,c,t,n] for n in self.N)) 
                                for i in self.I for t in self.T for c in self.CI if c > 0),
                                name='1C_ints')
            
            
            model.addConstrs((sum(xF[n, i, i, c, t % 2, t - t % 2] for n in self.N if (n, i, i, c, t % 2, t - t % 2) in self.xF_index) 
                                >= float(sum(self.hold_matrix[i,c+self.number_int_payloads,t,n] for n in self.N))
                                for i in self.I for t in self.T for c in self.CF),
                                name='1C_cont')
        return model
    ...
    def add_capacity_constrs(self,
        ...
        # Capacities
        # Oxidiser
        model.addConstrs((xF[n, i, j, 4, 0, t] <= self.spacecraft.oxy_ratio[n]*xI[n, i, j, 0, 0, t]*self.spacecraft.prop_cap[n]
                         for n in self.N for i in self.I for j in self.I for t in self.T 
                         if (n, i, j, 4, 0, t) in self.xF_index),
                         name='3A')
                                          
        # Fuel
        model.addConstrs((xF[n, i, j, 5, 0, t] <= (1-self.spacecraft.oxy_ratio[n])*xI[n, i, j, 0, 0, t]*self.spacecraft.prop_cap[n]
                         for n in self.N for i in self.I for j in self.I for t in self.T 
                         if (n, i, j, 5, 0, t) in self.xF_index),
                         name='3B')
                                            
        # Payload
        model.addConstrs((100*xI[n, i, j, 1, 0, t] + xF[n, i, j, 0, 0, t] + xF[n, i, j, 1, 0, t] + xF[n, i, j, 2, 0, t] + xF[n, i, j, 3, 0, t]
                        <= xI[n, i, j, 0, 0, t] * self.spacecraft.payload_cap[n]
                        for n in self.N for i in self.I for j in self.I for t in self.T 
                        if (n, i, j, 1, 0, t) in self.xI_index and (n, i, j, 0, 0, t) in self.xI_index 
                        and (n, i, j, 0, 0, t) in self.xF_index and (n, i, j, 1, 0, t) in self.xF_index and (n, i, j, 2, 0, t) in self.xF_index and (n, i, j, 3, 0, t) in self.xF_index
                        and (i != j or i not in (2,4))),
                        name='3C')
        model.addConstrs((100*xI[n, i, i, 1, 0, t] + xF[n, i, i, 1, 0, t] + xF[n, i, i, 2, 0, t] + xF[n, i, i, 3, 0, t]    
                        <= xI[n, i, i, 0, 0, t] * self.spacecraft.payload_cap[n]
                        for n in self.N for t in self.T for i in (2,4)
                        if (n, i, i, 1, 0, t) in self.xI_index and (n, i, i, 0, 0, t) in self.xI_index
                        and (n, i, i, 1, 0, t) in self.xF_index and (n, i, i, 2, 0, t) in self.xF_index and (n, i, i, 3, 0, t) in self.xF_index),
                        name='3C_moon_surface_and_NRHO')
        return model
    ...
    def add_commodity_dynamics_constrs(self,
        ...
        # Dynamics
        model.addConstrs((xI[n, i, j, c, 1, t] == xI[n, i, j, c, 0, t] 
                        for n in self.N for i in self.I for j in self.I for c in self.CI for t in self.T
                        if (n, i, j, c, 1, t) in self.xI_index and (n, i, j, c, 0, t) in self.xI_index),
                        name='4A_int')
        
        # Maintenance 
        model.addConstrs((sum(xF[n, i, i, 1, 1, t] for n in self.N if (n, i, i, 1, 1, t) in self.xF_index) 
                        == sum(xF[n, i, i, 1, 0, t] - (self.infra_maintenance*self.arc_real_time[n,i,i,t])*xF[n, i, i, 0, 0, t] for n in self.N if (n, i, i, 1, 0, t) in self.xF_index and (n, i, i, 0, 0, t) in self.xF_index)
                        for t in self.T for i in (2,4)),
                        name="4_maintenance_moon")
        model.addConstrs((xF[n, i, j, 1, 1, t] == xF[n, i, j, 1, 0, t]
                        for n in self.N for i in self.I for j in self.I for t in self.T 
                        if (n, i, j, 1, 1, t) in self.xF_index and (n, i, j, 1, 0, t) in self.xF_index and (i != j or i not in (2,4))),
                        name='4_maintenance_other')
        # Consumable 
        model.addConstrs((xF[n, i, j, 2, 1, t] == xF[n, i, j, 2, 0, t] - self.consumption_cost*self.arc_real_time[n,i,j,t]*xI[n, i, j, 1, 0, t]
                        for n in self.N for i in self.I for j in self.I for t in self.T
                        if (n, i, j, 2, 1, t) in self.xF_index and (n, i, j, 2, 0, t) in self.xF_index and (n, i, j, 1, 0, t) in self.xI_index),
                        name='4_consumables')
        # Oxygen
        model.addConstrs((xF[n, i, j, 4, 1, t] 
                        <= ((1-self.spacecraft.oxy_boil_off_rate[n])**self.arc_real_time[n,i,j,t])*(xF[n, i, j, 4, 0, t]
                            - (self.spacecraft.oxy_ratio[n])*self.arc_FPI[n,i,j,t]*(100*xI[n, i, j, 1, 0, t]                          
                            + xF[n, i, j, 0, 0, t] + xF[n, i, j, 1, 0, t] + xF[n, i, j, 2, 0, t]
                            + xF[n, i, j, 3, 0, t] + xF[n, i, j, 4, 0, t] + xF[n, i, j, 5, 0, t]  + self.spacecraft.dry_mass[n]*xI[n, i, j, 0, 0, t]))
                        for n in self.N for i in self.I for j in self.I for t in self.T
                        if (n, i, j, 5, 1, t) in self.xF_index and (n, i, j, 5, 0, t) in self.xF_index and (n, i, j, 1, 0, t) in self.xI_index
                        and (n, i, j, 0, 0, t) in self.xF_index and (n, i, j, 1, 0, t) in self.xF_index and (n, i, j, 2, 0, t) in self.xF_index
                        and (n, i, j, 3, 0, t) in self.xF_index and (n, i, j, 4, 0, t) in self.xF_index),
                        name='4_oxy')
        # Fuel 
        model.addConstrs((xF[n, i, j, 5, 1, t] 
                        <= ((1-self.spacecraft.fuel_boil_off_rate[n])**self.arc_real_time[n,i,j,t])*(xF[n, i, j, 5, 0, t]
                            - (1-self.spacecraft.oxy_ratio[n])*self.arc_FPI[n,i,j,t]*(100*xI[n, i, j, 1, 0, t]                          
                            + xF[n, i, j, 0, 0, t] + xF[n, i, j, 1, 0, t] + xF[n, i, j, 2, 0, t]
                            + xF[n, i, j, 3, 0, t] + xF[n, i, j, 4, 0, t] + xF[n, i, j, 5, 0, t]  + self.spacecraft.dry_mass[n]*xI[n, i, j, 0, 0, t]))
                        for n in self.N for i in self.I for j in self.I for t in self.T
                        if (n, i, j, 5, 1, t) in self.xF_index and (n, i, j, 5, 0, t) in self.xF_index and (n, i, j, 1, 0, t) in self.xI_index
                        ...
                        name='4_fuel')
        # Other cont. payloads
        model.addConstrs((xF[n, i, j, c, 1, t] == xF[n, i, j, c, 0, t]
                        for n in self.N for i in self.I for j in self.I for t in self.T for c in (0, 3)
                        if (n, i, j, c, 1, t) in self.xF_index and (n, i, j, c, 0, t) in self.xF_index),
                        name='4_other_cont')   
        
        # No crew on non-crew rated vehicles
        model.addConstrs((xI[n, i, j, 1, 0, t] == 0 
                        for n in [0,1,2,3,4] for i in self.I for j in self.I for t in self.T
                        if (n, i, j, 1, 0, t) in self.xI_index),
                        name='4_no_crew')

        return model

    def model_results_to_excel(self, model, output:str) -> None:
        '''
                ...
                for i in self.I:
                    for j in self.I:
                        if self.valid_arc[n, i, j, t]:
                            payload_list = []
                            for c in self.C:
                                if (n,i,j,c,0,t) in self.xI_index:
                                    payload_list.append(pd.DataFrame({c: model.getVarByName("xI[%d,%d,%d,%d,%d,%d]" % (n,i,j,c,0,t)).X}, index=[t]))
                                elif (n,i,j,c-len(self.CI),0,t) in self.xF_index:
                                    payload_list.append(pd.DataFrame({c: model.getVarByName("xF[%d,%d,%d,%d,%d,%d]" % (n,i,j,c-len(self.CI),0,t)).X}, index=[t]))
                            node_keys.append(str([i, j]))
                            if payload_list != []:
                                node_list.append(pd.concat(payload_list, axis=1))
                time_list.append(pd.concat(node_list, keys=node_keys, axis=1))
